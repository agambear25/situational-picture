...
def _composite_tile(ee, bbox, start, end, scale_m, granule_id) -> Tile:
    ...
    t = datetime.fromisoformat(end + "T00:00:00+00:00").astimezone(timezone.utc)
...
def get_tile(ee, city, bbox, start, end, scale_m, granule_id) -> Tile:
    ...
    key = hashlib.sha256(f"{city}|{start}|{end}|{scale_m}".encode()).hexdigest()[:16]
    path = _TILE_CACHE / f"sar_{key}.pkl"
    if path.exists():
        with path.open("rb") as f:
            return pickle.load(f)
    tile = _composite_tile(ee, bbox, start, end, scale_m, granule_id)
